...
PairLJCutBump::PairLJCutBump(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 1;
  writedata = 1;
...
void PairLJCutBump::coeff(int narg, char **arg)
{
  if (narg < 7 || narg > 8)
...
void PairLJCutBump::write_restart(FILE *fp)
{
...
void PairLJCutBump::read_restart(FILE *fp)
{
...
void PairLJCutBump::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairLJCutBump::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
